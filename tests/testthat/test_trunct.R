...
test_that("comp_postmean2 is 0 for null", {
  set.seed(1)
  z=rnorm(10)
  expect_equal(comp_postmean2(unimix(1,0,0),list(x=z,s=rep(1,10),v=NULL)),matrix(0,10,nrow=1))
  expect_equal(comp_postmean2(unimix(1,0,0),list(x=z,s=rep(1,10),v=4)), matrix(0,10,nrow=1))
  expect_equal(comp_postmean2(normalmix(1,0,0),list(x=z,s=rep(1,10),v=NULL)),matrix(0,10,nrow=1))
...
test_that("comp_postmean2.unimix matches simulations", {
  ...
  temp2=as.vector(comp_postmean2(g,list(x=c(3),s=c(2),v=3)))
  temp = as.vector(comp_postmean(g,list(x=3,s=2,v=3)))
