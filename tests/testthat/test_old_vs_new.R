...
# set.seed(1); z=rnorm(100,0,2); z.ash=ash(z,1); saveRDS(z.ash,file="tests/testthat/z.ash.test")
  ...
  expect_equal(get_pm(oldres),get_pm(z.ash))
  expect_equal(get_psd(oldres),get_psd(z.ash))
  expect_equal(get_lfsr(oldres),get_lfsr(z.ash))
  expect_equal(get_fitted_g(oldres),get_fitted_g(z.ash))
  expect_equal(get_logLR(oldres),get_logLR(z.ash))
  expect_equal(get_loglik(oldres),get_loglik(z.ash))
})
